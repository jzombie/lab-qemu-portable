...
Portable, zero-install builds of **[QEMU](https://www.qemu.org)**, **[wimlib](https://wimlib.net)**, and **virtio drivers** for Windows, Linux, and macOS. Two independent GitHub Actions pipelines compile pinned upstream sources per OS/arch and publish folder distributions (ZIP/TAR) as release assets. No installer, no admin rights: extract and run (on macOS you only need to clear the download quarantine flag, see below).
